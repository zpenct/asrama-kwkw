    ...
    return $form
      ->schema([
        ...
        Forms\Components\Repeater::make('floors')
          ...
          "Lantai {$state['floor']} - Kapasitas: {$state['max_capacity']}, Harga: Rp" . number_format($state['price'] ?? 0))
          ->schema([
          ...
          ->deleteAction(
            fn(Action $action): Action => $action
              ...
              ->modalDescription(fn(array $state): string =>
              'Terdapat ' . (isset($state['rooms']) ? count($state['rooms']) : 0) .
